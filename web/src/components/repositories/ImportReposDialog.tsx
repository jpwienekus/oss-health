...
import { GET_REPOS_FROM_GITHUB } from '@/graphql/queries'
...
export const ImportReposDialog = ({ onConfirm }: ImportReposDialogParams) => {
  ...
  const fetchGitHubRepos = async () => {
    ...
    const response = await client.request<{ getReposFromGithub: GitHubRepo[] }>(
      GET_REPOS_FROM_GITHUB,
    )
    setGitHubRepos(response.getReposFromGithub)
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <DialogFooter className="flex gap-2">
          ...
          <Button onClick={handleConfirm} disabled={selectedRepos.length === 0}>
            Confirm
          </Button>
        </DialogFooter>
      </DialogContent>
