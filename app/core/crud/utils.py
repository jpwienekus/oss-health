...
async def paginate_query(
    ...
    after_value = coerce_cursor_value(sort_column, decode_cursor(after)) if after else None
    before_value = coerce_cursor_value(sort_column, decode_cursor(before)) if before else None
    actual_sort_desc = descending ^ bool(before)  # XOR: invert if before

    if after_value and not before_value:
        filter_clause = sort_column < after_value if descending else sort_column > after_value
    elif before_value and not after_value:
        filter_clause = sort_column < before_value if actual_sort_desc else sort_column > before_value
    elif after_value and before_value:
        raise ValueError("Cannot use both 'after' and 'before' at the same time.")
    else: 
        filter_clause = None

    query = base_query.order_by(desc(sort_column) if actual_sort_desc else asc(sort_column))
    ...
    start_cursor = encode_cursor(getattr(paginated_items[0], sort_column.key)) if paginated_items else None
    end_cursor = encode_cursor(getattr(paginated_items[-1], sort_column.key)) if paginated_items else None
...
async def has_records_before(
    ...
    filter_clause = (
        sort_column > value if actual_sort_desc else sort_column < value
    )
